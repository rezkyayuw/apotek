	...
	public function index($rowno=0){
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$data['users'] = $datauser;
				$data['page'] = 'admin/kategori/main-v';
		        // Search text
		        $search_text = "";
				if($post == TRUE ){
					$search_text = $post;
		        ...
		        // All records count
				$allcount = $this->Kategori_m->getrecordCount($search_text);
		        // Get records
				$users_record = $this->Kategori_m->getData($rowno,$rowperpage,$search_text);
	...
	public function detail($id){
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$datauser = $this->ion_auth->user()->row();
				$perusahaan = $this->Admin_m->perusahaan(1);
				$detaildata = $this->Kategori_m->detail($id);
				$totalobathabis = $this->Admin_m->totalobathabis();
				$totalobatkedaluwarsa = $this->Admin_m->totalobatkedaluwarsa();
				$data['totalobathabis'] = $totalobathabis->jumlah;
				$data['totalobatkedaluwarsa'] = $totalobatkedaluwarsa->jumlah;
				...
				$data['page'] = 'admin/kategori/edit-v';
		        // echo "<pre>";print_r($users_record);echo "<pre>";exit();
				$this->load->view('admin/dashboard-v', $data);
			}
		}else{
	...
	public function tambah(){
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$data['perusahaan'] = $perusahaan;
				$data['users'] = $datauser;
				$data['page'] = 'admin/kategori/tambah-v';
		        // echo "<pre>";print_r($users_record);echo "<pre>";exit();
				$this->load->view('admin/dashboard-v', $data);
			}
		}else{
		...
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				// $id = $this->input->post('id_unit');
					...
					'des_kategori' => $this->input->post('des_kategori'),
				);
				$this->Kategori_m->insert($additional_data);
				$pesan = 'Kategori '.$this->input->post('nama_kategori').' berhasil di tambahkan';
				$this->session->set_flashdata('message', $pesan );
	...
	public function proses_edit(){
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$this->Kategori_m->update($id,$additional_data);
				$pesan = 'Kategori '.$this->input->post('nama_kategori').' berhasil di edit';
				$this->session->set_flashdata('message', $pesan );
				redirect(base_url('index.php/kategori'));
			}
		}else{
